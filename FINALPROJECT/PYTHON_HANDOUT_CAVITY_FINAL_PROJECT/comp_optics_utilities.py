...
def propagate_fresnel_transfer_2d(coordinates, A, photon_lambda, L, H = "None"):
    ...
    #If the transfer function was provided, use it rather than waste computation generating it again
    H_fresnel = None
...
#np.abs(coordinates[0][0][0] - coordinates[1][1][0]) is dx (in computer science frequency indexing, in phys it would go +/-)
    ...
    Ap = Ap/np.sqrt(np.sum(np.square(Ap)))
