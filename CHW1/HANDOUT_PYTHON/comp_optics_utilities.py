...
def plot_intensity_1d(samples_coordinates, A, name, xunits = "millimeter"):
    ...
    plt.stem(samples_plot_coordinates, np.abs(A)**2, markerfmt = 'x', basefmt=" ", label = "sampled points")
...
def propagate_fraunhofer(coordinates, A, _lambda, L):
    ...
    propagated_amplitude = normalize(np.fft.fftshift((np.fft.fft(np.fft.fftshift(A)))))
